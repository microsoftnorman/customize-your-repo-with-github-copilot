...
In VS Code, hooks can be created from the UI or from chat. Use `/hooks`, run **Chat: Configure Hooks**, or open **Hooks** from the Chat settings menu to create or edit a hook file. Use `/create-hook` when the team wants GitHub Copilot to draft the JSON from a plain-English description.
