...
- **Android Studio**. Built on IntelliJ IDEA Community; receives the same plugin and the same customization primitives.
...
The JetBrains plugin exposes several top-level capabilities. Each one is a direct analog of the same capability in VS Code. Model selection, the tool-calling infrastructure, and the on-disk configuration files for the primitives discussed further down are all the same.
...
Per-language completion toggles live at **Settings → Languages & Frameworks → GitHub Copilot**, which is useful for silencing completions in, say, SQL files while keeping them on in Kotlin.
...
This is also the recommended path for support tickets. GitHub Support (`support.github.com`) handles account, billing, and service-wide issues; plugin bugs belong in the public feedback repo.

## JetBrains AI Assistant vs. GitHub Copilot

JetBrains ships its own AI product in the same IDEs: **JetBrains AI Assistant**. It is a separate plugin (`com.intellij.ml.llm`) with its own subscription sold by JetBrains and its own chat and completions surface. It can coexist with GitHub Copilot in the same IDE, but the two do not share configuration, history, or model selection. Users encountering a chat panel that doesn't understand `.github/copilot-instructions.md` are usually looking at AI Assistant's panel, not Copilot's. The Copilot panel is labelled "GitHub Copilot" and lives under the right tool window bar; AI Assistant has its own icon with a different gradient. Disable whichever plugin is not in use to avoid double-billing completion shortcuts.
