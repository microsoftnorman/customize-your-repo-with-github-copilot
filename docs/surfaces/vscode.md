...
- **Customize Your Agent (natural-language generator).** Describe an agent in plain English and VS Code generates a draft `.agent.md` — complementary to `/create-agent`. See [Custom Agents → Authoring Paths in VS Code](../primitive-5-custom-agents.md#authoring-paths-in-vs-code).
