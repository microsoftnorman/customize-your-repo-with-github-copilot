...
Tool names are host-specific. The example above uses current VS Code tool IDs. Do not treat them as portable canonical names across GitHub.com, GitHub Copilot CLI, or other IDE surfaces.
...
| `metadata` | No | object | Cloud-agent metadata annotations |
| `hooks` | No | object | Agent-scoped hooks (Preview) |

**Compatibility note:** `argument-hint` and `handoffs` are IDE-facing fields and are currently ignored on GitHub.com. `mcp-servers` and `metadata` are cloud-agent fields and are not used by VS Code and other IDE custom agents.
