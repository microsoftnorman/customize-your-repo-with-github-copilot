...
This content has been consolidated into [part-3-reference.md](part-3-reference.md), which serves as the single lookup layer for file locations, load order, frontmatter fields, surface support, and operational tables.

[← Back to Guide](README.md) | [← Executive Maturity Model](executive-maturity-model.md) | [Next: Appendix: Diagnosing Loop Failures →](appendix-diagnosing-loop-failures.md)
...
Use this table when the problem is already clear and the question is selection, not theory.
...
The same choice can also be expressed in the negative:

- If the team is changing defaults, start with instructions.
- If the team is packaging a reusable task, start with prompts, skills, or agents.
- If the team needs new reach, start with MCP.
- If the team needs runtime enforcement, start with Hooks.
- If the team wants the system to retain learned repository patterns over time, start with Memory.
