...
[← Back to Guide](README.md) | [← The Eight Primitives](part-2-primitives.md) | [Next: Operational Reference →](part-3-reference.md)
...
- **Hook-driven logging.** Pipe [hooks](primitive-7-hooks.md) audit output to your SIEM for a per-session view of what GitHub Copilot did in high-risk repos.

### Operational Telemetry

Business outcomes explain whether the rollout mattered. Operational telemetry explains why it is working or failing.

| Signal | What it reveals |
|--------|-----------------|
| Approval interruption rate | Whether the runtime is demanding too much manual confirmation to stay useful |
| Hook deny rate and false-positive rate | Whether enforcement is catching real problems or just blocking normal work |
| MCP startup or server failure rate | Whether live-system integrations are actually reliable |
| Customization load errors | Whether the repo files are being discovered and parsed consistently |
| Environment bootstrap success | Whether Codespaces, dev containers, or fresh machines can reproduce the setup |
| Headless or delegated task success rate | Whether automation works outside a supervised local chat session |
| Drift rate and sync lag | Whether the baseline is staying coherent across many repos |

These are not vanity metrics. They are debugging metrics.

### Governance Checks

Leadership needs explicit expansion and stop rules.

- `Expand` when the target workflow improves, support cost stays reasonable, and false positives are low enough that the team still trusts the system.
- `Pause` when preview-only features are carrying too much of the workflow, or when the support burden starts rising faster than the outcome improves.
- `Stop or redesign` when the rollout mostly shifts work around instead of removing it.

Three boring questions keep teams honest:

- What is the kill criterion for this pilot?
- What support cost is acceptable per team?
- What counts as real gain rather than novelty, displacement, or hidden manual cleanup?

### Adjust the Scorecard to the Operating Model

| Team shape | Add these measures |
|------------|--------------------|
| Frontend or design-system work | Accessibility regressions, visual regression escape rate, component reuse, UI review churn |
| Data-sensitive workflows | Blocked unsafe queries, prevented PII exposure, approval rate for production-data access, notebook reproducibility |
| Terminal-first or remote-heavy teams | Shell workflow reuse, headless task success, remote task completion, bootstrap success |
| OSS maintainers | Time to first conforming PR, repeated review comments, customization-file churn, fork drift |
| Mobile teams | Distinguish merge-to-release from store availability; track TestFlight or internal-distribution lag separately |

### Phased Rollout Across the Organization

Concentric waves build signal before scale:

| Wave | Scope | Exit Criteria |
|------|-------|---------------|
| **Initial** | 1-2 volunteer teams on active repos | Friction down, PR rework down; baseline instructions and 2-3 skills stable |
| **Wave 1** | 5-10 teams, mixed tech stacks | Customizations survive contact with different stacks; team knows which rules generalize |
| **Wave 2** | Department or business unit | Distribution mechanism working; CODEOWNERS in place; drift detection live |
| **Wave 3** | Entire organization | Metrics API dashboards running; audit logging integrated; support rotation staffed |

At each wave, collect qualitative feedback and trim. The biggest org-wide rollout failure mode is accumulating every team's rules into a shared file that satisfies no one. Keep the baseline lean; let teams extend it locally.
